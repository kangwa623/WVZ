...
const TripLogger: React.FC<TripLoggerProps> = ({
  ...
  const handleStartTrip = async () => {
    if (!startMileage || !currentLocation || !passengerCount || !destination || !purpose) {
      Alert.alert('Error', 'Please fill in all required fields');
      return;
    }

    // For MVP, bookingId is optional - use mock booking if not provided
    const finalBookingId = bookingId || 'mock_booking_1';
    ...
    try {
      ...
      // Navigate immediately without waiting - storage operations happen in background
      router.push({
        pathname: '/trips/active',
        params: { 
          tripId: trip.id,
          tripData: JSON.stringify(trip), // Pass trip data directly for immediate display
        },
      });
    } catch (error) {
  ...
  const handleStopTrip = async () => {
    ...
    // Don't use loading state - update UI immediately
    try {
